...
{
    public async Task<ProductionResult> RunOnceAsync(CancellationToken token = default)
    {
        ...
        string facts = JsonSerializer.Serialize(new
        {
            ...
            agent = new { alive = agent.GetProperty("alive"), health = agent.GetProperty("health"),
                inventory = agent.GetProperty("inventory"), ammoRounds = agent.GetProperty("ammoRounds") },
            ...
            executionCapabilities = "Current grounder supports production goals for a solid item stock carried by the actor, up to 1000 items per goal. " +
                "It can explore, mine, hand-craft and feed a burner furnace. Other meaningful goals may still be proposed and will return an explicit unsupported result. " +
                "No continuous production, research execution, automatic machine prerequisite construction or fluid networks are implemented yet.",
        ...
        return await new ProductionController(game, journal).ProduceAsync(goal.Target, (int)goal.Quantity, token);
    }
