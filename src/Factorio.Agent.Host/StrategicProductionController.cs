    ...
    public async Task<StrategicGoalResult> RunOnceAsync(CancellationToken token = default, string? previousResult = null)
    {
        ...
        string facts = JsonSerializer.Serialize(new
        {
            ...
            executionCapabilities = "Production goals use category production, unit items and an exact native item identifier, up to 1000 carried items. " +
                ...
                "Fluid production goals use category production, unit fluid_units and an exact native fluid identifier, up to 100000 units in the known factory. C# supports one-fluid-input/one-fluid-output conversion with an existing input source, native refinery configuration and ordinary pipe routes in the observed construction area. Solid-item goals may combine deterministic item and fluid inputs in compatible machines, with finite fluid preparation and native pipe connections. Long-distance fluid networks, mixed inputs producing fluids, temperature-constrained chemistry and industrial item transport remain unsupported. " +
