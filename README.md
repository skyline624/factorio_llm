...
La première commande reçoit un stock cible explicite dans l'inventaire du personnage. La seconde demande un objectif libre au modèle cloud puis vérifie s'il est actuellement exécutable. Les deux privilégient en C# les foreuses et les fours après lecture des stocks et de la géométrie. La production générale peut réutiliser une installation ou fabriquer puis placer conjointement les machines manquantes ; le minage manuel reste utilisé pour l’amorçage et les ressources encore non couvertes. Voir les [preuves de cette priorité](docs/mining-priority.md). Les demandes de charbon et de pierre passent également par une [foreuse alimentant un coffre](docs/raw-extraction.md), avec mesure du stock net après consommation de combustible. `automate-smelting` permet également de demander explicitement le chemin foreuse–four. Celui-ci exige un gisement adapté autour d'un four déjà construit ; le personnage ravitaille les machines et collecte leurs produits. Une erreur d'exécution n'entraîne pas de basculement automatique vers une autre méthode. Cette boucle ne synthétise pas encore une chaîne automatisée jusqu'à la fusée. Voir les [capacités et preuves de production](docs/production.md).
...
Le manifeste contient un secret RCON local et reste ignoré par Git. Le serveur écoute sur la boucle locale. Les commandes conservent une [connexion RCON sérialisée et explicitement fermée](docs/rcon-lifetime.md), pour éviter les ouvertures répétées pendant les observations. Un résultat de mutation inconnu exige consultation de son reçu ; aucune répétition automatique n'est effectuée. La [réconciliation stratégique](docs/strategic-campaign.md) vérifie les effets de l’objectif interrompu avant une nouvelle décision ; la [récupération après mort](docs/death-recovery.md) a été vérifiée dans des scénarios préparés. Les campagnes complètes restent à qualifier. Voir les [résultats vérifiés et limites](docs/validation.md).
