...
{
    [Fact]
    public async Task ArmedCraftFixtureCannotModifyANormalCampaign()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => new ArmedCraftingQualification(session).RunAsync(CancellationToken.None));
    }

    [Fact]
    public async Task PreparedDefenseDeploymentCannotModifyANormalCampaign()
    {
        var session = new RuntimeSession("nonexistent-normal-campaign", "factorio.exe", "config.ini", "mods", "save.zip",
            0, 1, 2, "test-only", "session", "world", 1, false);
        await Assert.ThrowsAsync<InvalidOperationException>(() => new DefenseDeploymentQualification(session).RunAsync(CancellationToken.None));
    }
