...
- `observe` : `{radius?:number=32,limit?:integer=100}` ; rayon 1 à 64, limite 1 à 200. Retour : `scope`, `snapshotId`, `collectedTick`, `agent`, `operation`, `entities`, `resources`, `enemies`, `players`, `coverage`, `goal`.
...
`entityId` est préférable aux coordonnées pour les cibles possédant un `unit_number`. Sinon `position` et le nom facultatif sélectionnent la cible la plus proche dans un rayon de 0,6 case. Les identifiants des ressources dépourvues de `unit_number` sont des références descriptives surface/nom/position ; elles doivent être ciblées par position.
...
La mort est un événement moteur. Le remplacement scripté attend `character.prototype.respawn_time`, puis reprend seulement les objets de respawn vanilla : pistolet et 10 chargeurs, sans réattribuer le matériel initial. Le cadavre et son contenu ne sont ni effacés ni copiés par le mod. Ce cycle doit être qualifié sur un personnage sans joueur et avec un pilote ; il ne sera pas présenté comme validé avant preuve. Une réapparition bloquée reste observable et est retentée après 600 ticks.
...
Ressources et obstacles naturels sont découverts dans le rayon local demandé ; troncature explicite. Les ennemis actuels sont filtrés par `force.is_chunk_visible`, jamais par le seul historique cartographique. Aucune commande de révélation (`chart_all`, etc.) n'est utilisée. La visibilité d'un personnage standalone et l'interaction des permissions avec un pilote connecté sont des points de qualification moteur obligatoires.
