...
- `factory_snapshot` : collecte atomique paginée des stocks propres connus ; contrat détaillé dans [factory-state.md](factory-state.md). Le scope courant de l'enveloppe reste distinct du scope de la photographie.
  Les métadonnées des tourelles à munitions incluent `ammoInventoryId`, `ammoRounds`, `active`, `quality`, `defenseReady` et, lorsque disponible, `defenseRange`. Le catalogue de production expose les catégories et tailles natives des chargeurs ainsi que les prototypes `turrets` pris en charge. Le compartiment `output` exclut les tourelles, dont les réserves sont accessibles par `ammo` ; elles ne sont pas des sorties de production. Voir [defense-deployment.md](defense-deployment.md).
...
| `craft` | `{recipe,count?:1..1000=1}` | Recette débloquée, file initialement vide, ingrédients directs et capacité du sac vérifiés. Quantité effectivement engagée dans `effects.queued`. L'achèvement exige file vide **et** produits corroborés par les compteurs natifs ; quantité engagée inférieure à la demande → `partial`. Les surplus auto-équipés sont transférés nativement vers le sac. Pour les munitions, `products` et le nombre net de chargeurs livrés peuvent différer après regroupement de piles partielles ; voir [craft-delivery.md](craft-delivery.md). Produits probabilistes et fluides refusés. |
