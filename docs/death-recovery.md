...
### Reprise du monde normal après qualification

La reprise avec le correctif publié `896ff9f` a conservé le marqueur de mort pendant récupération, mais s’est arrêtée avant la nouvelle stratégie : parmi les 219 soumissions du journal de récupération interrompu, 218 possèdent un reçu terminal et le dernier déplacement n’a aucun reçu durable. Sa requête native renvoie `operation_unknown`. L’observation retrouve le déplacement précédent terminé ; elle ne prouve pas le résultat de la soumission manquante. Aucun ordre n’a été rejoué et aucun succès n’a été supposé. Le serveur a été sauvegardé et arrêté. L’observation au tick **9553257** constate toujours l’incarnation 11, dix morts, 250 points de vie et zéro fusée. Il reste à distinguer une soumission jamais acceptée d’un reçu absent ou expiré par une preuve native suffisante avant de poursuivre cette partie.
